...
def main(argv):
	...
	opts, ars = getopt.getopt(argv, "n:d:t:l:e:s:T:", ["runID=", "directory=", "target=", "length=", "error=", "numsim=", "tpr="])
	...
	for i in range(int(num_sim)):
		...
		grid_sweep=len(lines_sweep)-1
		grid_neut=len(lines_neut)-1
		#print(grid_sweep)
		#print(probs)
		pro_sweep=[]
		pro_neut=[]
		for j in range(int(grid_sweep)):
...
#	print(ans)
	...
	#print(int(float(float(target)-float(error))*int(length)))
	for i in range(int(num_sim)):
		for j in range(int(len(pos_list_sweep[i]))):
