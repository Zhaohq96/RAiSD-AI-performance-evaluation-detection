...
# Run diploSHIC
	...
	python3 diploshic_detection.py -i "$4"window_"$diploSHIC_win"/neut_positions.ms -I "$4"window_"$diploSHIC_win"/sweep_positions.ms -d $4 -g $5 -s $num_scan -e $error -o $4 -T $tpr >> "$4"Detection_result.txt
	conda deactivate
...
# Run T-REx
	...
	python3 t-rex_detection.py -i "$4"Results/sim_positions.ms -d "$4"Results/ -e $error -s $num_scan -g $5 -o "$4"Results/ -T $tpr >> "$4"Results/Results.txt
	conda deactivate
...
# Run T-REx
	...
	conda activate T-REx
	bash scan_dataset_trex_ori.sh -n "$2"neutral.ms -s "$2"selsweep.ms -N "$3"neutral.ms -S "$3"selsweep.ms -o $4 -w $T_REx_win -l $length -t $num_train -T $num_scan -r $rank -m $mbs
	#python3 t-rex_detection.py -i "$4"Results/sim_positions.ms -d "$4"Results/ -e $error -s $num_scan -g $5 -o "$4"Results/ -T $tpr >> "$4"Results/Results.txt
	conda deactivate
...
# Run FASTER-NN
	...
	python3 raisd-ai_detection.py -n "$7" -d $4 -g $5 -s $num_scan -l $length -e $error -T $tpr >> "$4"Results.txt
	conda deactivate
