...
num_train=2000 # number of simulations of each class of training sets, note that the training sets should be balanced
...
#num_scan=2 # number of simulations for inference
error=0.01 # error
...
elif [ "$6" = "low-intensity-recombination-hotspot-1K" ] || [ "$6" = "low-intensity-recombination-hotspot-5K" ] || [ "$6" = "low-intensity-recombination-hotspot-10K" ] || [ "$6" = "high-intensity-recombination-hotspot-merge-2K" ]; then
	rank=300
...
elif [ "$6" = "low-intensity-recombination-hotspot-merge-2K" ]; then
	rank=150
...
# Run diploSHIC
	...
	bash scan_dataset_diploshic.sh "$3"neutral.ms "$3"selsweep.ms $4 $2 $diploSHIC_win $length $5 $6
	python3 diploshic_detection.py -i "$4"window_"$diploSHIC_win"/neut_positions.ms -I "$4"window_"$diploSHIC_win"/sweep_positions.ms -d $4 -g $5 -s $6 -e $error -o $4 -T $tpr >> "$4"Detection_result.txt
	conda deactivate
...
# Run T-REx
	...
	bash scan_dataset_t-rex.sh -n "$2"neutral.ms -s "$2"selsweep.ms -N "$3"neutral.ms -S "$3"selsweep.ms -o $4 -w $T_REx_win -l $length -g $5 -t $num_train -T $7 -r $rank
	python3 t-rex_detection.py -i "$4"Results/sim_positions.ms -d "$4"Results/ -e $error -s $7 -g $5 -o "$4"Results/ -T $tpr >> "$4"Results/Results.txt
	conda deactivate
	
elif [ "$1" = "t-rex-ori" ]; then
# Run T-REx
	if [ "$5" = "high-intensity-recombination-hotspot" ] || [ "$5" = "low-intensity-recombination-hotspot" ]; then
		mbs=1
	fi
	conda activate T-REx
	bash scan_dataset_trex_ori.sh -n "$2"neutral.ms -s "$2"selsweep.ms -N "$3"neutral.ms -S "$3"selsweep.ms -o $4 -w $T_REx_win -l $length -t $num_train -T $6 -r $rank -m $mbs
	python3 trex_ori_detection.py -d "$4"Results/ -s $6 -t $target -T $tpr -l $length -e $error >> "$4"Results/Results.txt
	conda deactivate
...
# Run FASTER-NN
	...
	python3 raisd-ai_detection.py -n "$7" -d $4 -g $5 -s $8 -l $length -e $error -T $tpr >> "$4"Results.txt
	conda deactivate

elif [ "$1" = "faster-nn-g-8" ]; then
# Run FASTER-NN-G-8
	conda activate raisd-ai
	bash scan_dataset_faster-nn-g8.sh $2 "$3" $4 $length $raisd_target $5 $error $7 $mbs
	neut="$4"RAiSD_Info."$7"_neut
	sweep="$4"RAiSD_Info."$7"_sweep
	faster_nn_inference_time=$(echo "scale=2; (($(tail -n 3 "$neut" | head -n 1 | awk '{print $4}') + $(tail -n 3 "$sweep" | head -n 1 | awk '{print $4}')) * 100) / 100" | bc)
	echo "Total execution time (sec):" > "$4"Results.txt
...
# Run RAiSD
	...
	bash scan_dataset_raisd.sh $2 "$3" $length $raisd_target $error $mbs $4
	neut="$3"RAiSD_Info."$4"_neut
	sweep="$3"RAiSD_Info."$4"_sweep
	echo "Total execution time (sec):" > "$3"Results.txt
	raisd_inference_time=$(echo "scale=2; (($(tail -n 3 "$neut" | head -n 1 | awk '{print $4}') + $(tail -n 3 "$sweep" | head -n 1 | awk '{print $4}')) * 100) / 100" | bc)
	echo "Total execution time (sec):" > "$4"Results.txt
	echo $raisd_inference_time >> "$3"Results.txt
	python3 raisd_detection.py -n "$4" -d $3 -s $num_scan -l $length -e $error -T $tpr >> "$3"Results.txt
