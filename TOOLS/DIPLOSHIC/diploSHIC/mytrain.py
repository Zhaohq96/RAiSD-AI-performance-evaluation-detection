...
if argsDict['mode'] == 'train':
    ...
    both=np.concatenate((h1,n1))
    y=np.concatenate((np.repeat(0,len(h1)),np.repeat(1,len(n1))))
    ...
    if (trainingDir==testingDir):
        ...
        both2=np.concatenate((h1,n1))
        X_test = both2.reshape(both2.shape[0],nDims,numSubWins,1)
        y_test=np.concatenate((np.repeat(0,len(h1)),np.repeat(1,len(n1))))
    ...
    print("total time spent fitting and evaluating: %f secs\n" %(time.time()-start))
    ...
    if argsDict['simData']:
        testX = x_df[list(x_df)[0:]].to_numpy()
    else:
