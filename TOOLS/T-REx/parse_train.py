...
parser.add_argument('cvf_folder',type=str, help= 'Input folder path')
...
for i in range(num_):
    ...
    df = pd.read_csv(input_folder+sub+"train_"+str(i+1)+".csv",header=None)
    ...
    pd.DataFrame(sym_sortedA).to_csv(cvf_folder+sub+"train_L1_"+str(i+1)+".csv")
    print(i)
...
# generalized
for i in range(num_):
    df=pd.read_csv(cvf_folder+sub+"train_L1_"+str(i+1)+".csv", index_col=0)
    df=np.asmatrix(df)
    s=np.shape(df)
    sortd=np.zeros(s)
    K=[]
    ...
    pd.DataFrame(d).to_csv(cvf_folder+sub+"train_align_"+str(i+1)+".csv")
#     print(i)
